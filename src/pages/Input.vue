  ...
  <q-page v-if="data">
    <q-layout view="lhh LpR lff" container class="bg-white window-height">
      <q-header class="bg-white" bordered>
        <q-toolbar>
          <q-toolbar-title>
            <div class="text-subtitle1 text-grey-10 text-weight-bold">Entrées du stock</div>
          </q-toolbar-title>
        ...
        <q-dialog v-model="confirm" persistent>
          <q-card>
            <q-card-section class="row">
              <span class="q-ml-sm q-pa-lg text-weight-bold text-center">Voulez vous vraiment valider cette entrée ?</span>
            </q-card-section>
            <q-card-section>
...
<script>
import inputComponent from 'components/inputOutputComponent'
      ...
      if (this.recordToEdit !== null) {
        ...
        this.$axios
          ...
          .then(res => {
            ...
            this.$q.notify({
              color: 'blue-10',
              message: "L'entrée a été validée avec succés"
            })
          })
          .catch(err => {
            this.confirm = false
