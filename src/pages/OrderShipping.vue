  ...
  <q-page v-if="data">
    <q-layout view="lhh LpR lff" container class="bg-white window-height">
      <q-header class="bg-white" bordered>
        <q-toolbar>
          <q-toolbar-title>
            <div class="text-subtitle1 text-grey-10 text-weight-bold">Bon de livraisons</div>
          </q-toolbar-title>
        </q-toolbar>
      </q-header>
      ...
      >
        <OrderShippingCard v-if="recordToEdit !== null" :selectedRow="recordToEdit"></OrderShippingCard>
        <EmptyCard v-else></EmptyCard>
      </q-drawer>
      <q-page-container>
        <q-page class>
          <q-table
            v-if="hasRole('GET_SHIPPING')"
          ...
          >
            ...
            <template v-slot:body-cell-code="props">
              <q-td @click="setRecordToEdit(props.row.id);">
                <div class="text-caption text-weight-bold">{{ props.row.shipping_code }}</div>
              </q-td>
            </template>
...
export default {
  ...
  methods: {
    ...
    getOrderShippings () {
      this.$axios.get('/orderShipping').then(res => {
        this.data = res.data
        this.fetched = true
      })
    },
    ...
    setRecordToEdit (id) {
      this.$axios.get('orderShipping/' + id).then(res => {
