      ...
      <q-form @submit="onSubmit" ref="form">
        <q-card-section class="bg-grey-1 no-padding">
          <div class="row q-pa-sm">
            <div class="col-6">
              <div class="text-subtitle1 q-pl-sm">Nouveau bon de retour</div>
            </div>
              ...
              >
                ...
                <template v-slot:body-cell-qty="props">
                  <q-td>
                    ...
                    >{{ props.row.delivred_qty - props.row.returned_qty }}</div>
                  </q-td>
                </template>
                <template v-slot:body-cell-qty_to_return="props">
                  <q-td>
                    <q-input
                      filled
                      dense
                      v-model="props.row.qty_to_return"
                      type="number"
...
export default {
  ...
  props: ['returnDialog', 'selectedRowReturn', 'order'],
  data () {
    return {
      loading: false,
      note: '',
  ...
  methods: {
    checkQty (row) {
      if (row.qty_to_return > row.delivred_qty - row.returned_qty) {
        row.qty_to_return = row.delivred_qty - row.returned_qty
      }
      if (row.qty_to_return < 0) {
        row.qty_to_return = 0
      }
    },
    hasRole (role) {
    ...
    onSubmit () {
      ...
      this.$refs.form.validate().then(success => {
        if (success) {
          this.$axios
            .post('/returnedOrder', {
              rows: this.selectedRowReturn.filter(
                el =>
                  el.qty_to_return &&
                  el.qty_to_return !== null &&
                  parseFloat(el.qty_to_return) > 0
              ),
              note: this.note
            })
            .then(res => {
              ...
              this.$q.notify({
                color: 'blue-10',
                message: 'Le bon de retour a été créé avec succés'
              })
